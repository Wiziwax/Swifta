...
public interface UserRepository extends JpaRepository<User, Integer>, QuerydslPredicateExecutor<User> {
    ...
    @Query(nativeQuery = true, value = "SELECT u.*, r.service_type_name FROM user u JOIN user_service_types ur ON u.user_id = ur.user_id JOIN service_type r ON ur.service_type_id = r.service_type_id WHERE r.service_type_id in (:serviceType) and u.address_user_id in (:addressIds)")
